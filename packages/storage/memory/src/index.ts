...
export class MemoryEventStore implements EventStore {
  ...
  private deliveries = new Map<string, Array<{ attempt: number; status: string; error?: string }>>()
  private dlq = new Map<string, string>()

  async put(event: EventEnvelope): Promise<void> {
    this.events.set(event.id, event)
    const key = this.dedupeKey(event.provider, event.connectionId, event.dedupeKey)
    this.dedupe.add(key)
  }
  ...
  async hasDedupe(provider: string, connectionId: string, dedupeKey: string): Promise<boolean> {
  ...
  async listDLQ(
    ...
    for (const [eventId, reason] of this.dlq.entries()) {
      ...
      if (filters?.connectionId && event.connectionId !== filters.connectionId) continue
  ...
  private dedupeKey(provider: string, connectionId: string, dedupeKey: string): string {
