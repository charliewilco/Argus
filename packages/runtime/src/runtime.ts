...
export class Runtime {
  ...
  constructor(opts: { eventStore: CoreEventStore; queue: CoreQueue; maxAttempts?: number }) {
  ...
  registerConnection(connection: CoreConnection): void {
    ...
    this.connections.set(this.connectionKey(connection.tenantId, connection.connectionId), connection)
  }
  ...
  async handleWebhook(input: {
    ...
    const connection = this.connections.get(this.connectionKey(input.tenantId, input.connectionId))
    if (!connection) {
    ...
    if (!trigger) {
      return { accepted: false, reason: `unknown trigger: ${input.triggerKey}` }
    }
  ...
  private async runDeliveryCycle(): Promise<void> {
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        await this.eventStore.markDelivery(event.id, job.attempt, "failed", error)
  ...
  private async persistAndQueue(events: EventEnvelope[], trigger: TriggerDefinition): Promise<void> {
    ...
    for (const event of events) {
      ...
      const id = await createEventId(event.provider, event.connectionId, dedupeKey)
  ...
  private resolveTrigger(provider: Provider, triggerKey: string): TriggerDefinition | undefined {
  ...
  private stateKey(connection: CoreConnection, trigger: TriggerDefinition): string {
